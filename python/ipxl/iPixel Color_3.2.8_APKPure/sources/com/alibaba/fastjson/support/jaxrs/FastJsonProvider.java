...
public class FastJsonProvider implements MessageBodyReader<Object>, MessageBodyWriter<Object> {
    ...
    public void writeTo(Object obj, Class<?> cls, Type type, Annotation[] annotationArr, MediaType mediaType, MultivaluedMap<String, Object> multivaluedMap, OutputStream outputStream) throws IOException, WebApplicationException {
        ...
        try {
            writeJSONStringWithFastJsonConfig(outputStream, locateConfigProvider.getCharset(), obj, locateConfigProvider.getSerializeConfig(), locateConfigProvider.getSerializeFilters(), locateConfigProvider.getDateFormat(), JSON.DEFAULT_GENERATE_FEATURE, locateConfigProvider.getSerializerFeatures());
            outputStream.flush();
    ...
    static final int writeJSONStringWithFastJsonConfig(OutputStream outputStream, Charset charset, Object obj, SerializeConfig serializeConfig, SerializeFilter[] serializeFilterArr, String str, int i, SerializerFeature... serializerFeatureArr) throws IOException {
        ...
        JSONWriter.Context createWriteContext = JSON.createWriteContext(serializeConfig, i, serializerFeatureArr);
        createWriteContext.setDateFormat(str);
        JSONWriter ofUTF8 = JSONWriter.ofUTF8(createWriteContext);
        if (serializeFilterArr != null) {
            try {
                for (SerializeFilter serializeFilter : serializeFilterArr) {
                    JSON.configFilter(createWriteContext, serializeFilter);
                }
            } catch (Throwable th) {
                if (ofUTF8 != null) {
                    try {
        ...
        ofUTF8.writeAny(obj);
        int flushTo = ofUTF8.flushTo(outputStream, charset);
        if (ofUTF8 != null) {
            ofUTF8.close();
        }
        return flushTo;
