...
class DomainValidator {
    ...
    public static boolean isValid(String str) {
        ...
        Matcher matcher = domainRegex.matcher(unicodeToASCII);
        if (!matcher.matches()) {
            return false;
        }
        ...
        return isValidTld(strArr[0]);
    }

    public static boolean isValidTld(String str) {
        String lowerCase = unicodeToASCII(unicodeToASCII(str)).toLowerCase(Locale.ENGLISH);
        if (lowerCase.startsWith(".")) {
            lowerCase = lowerCase.substring(1);
        }
        return Arrays.binarySearch(INFRASTRUCTURE_TLDS, lowerCase) >= 0 || Arrays.binarySearch(GENERIC_TLDS, lowerCase) >= 0 || Arrays.binarySearch(COUNTRY_CODE_TLDS, lowerCase) >= 0;
    }
