...
final class ObjectReaderImplOffsetDateTime extends DateTimeCodec implements ObjectReader {
    ...
    public Object readObject(JSONReader jSONReader, Type type, Object obj, long j) {
        ...
        if (jSONReader.isInt()) {
            ...
            return OffsetDateTime.of(LocalDateTime.ofInstant(ofEpochMilli, zoneId), zoneId.getRules().getOffset(ofEpochMilli));
        }
        ...
        if (this.formatMillis || this.formatUnixTime) {
            ...
            return OffsetDateTime.of(LocalDateTime.ofInstant(ofEpochMilli2, zoneId2), zoneId2.getRules().getOffset(ofEpochMilli2));
        }
        ...
        if (!this.formatHasHour) {
            ...
            return OffsetDateTime.of(of, zoneId2.getRules().getOffset(of));
        }
        if (!this.formatHasDay) {
            return ZonedDateTime.of(LocalDate.of(1970, 1, 1), LocalTime.parse(readString, dateFormatter), zoneId2).toOffsetDateTime();
        }
        LocalDateTime parse = LocalDateTime.parse(readString, dateFormatter);
        return OffsetDateTime.of(parse, zoneId2.getRules().getOffset(parse));
    }
}
