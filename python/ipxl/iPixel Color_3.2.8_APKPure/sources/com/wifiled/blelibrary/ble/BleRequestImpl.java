...
public final class BleRequestImpl<T extends BleDevice> {
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() { // from class: com.wifiled.blelibrary.ble.BleRequestImpl.1
        ...
        public void onCharacteristicWrite(BluetoothGatt bluetoothGatt, BluetoothGattCharacteristic bluetoothGattCharacteristic, int i) {
            ...
            synchronized (BleRequestImpl.this.locker) {
                BleDevice bleDeviceInternal = BleRequestImpl.this.getBleDeviceInternal(bluetoothGatt.getDevice().getAddress());
                if (i == 0) {
                    BleLog.d(BleRequestImpl.TAG, "notifyWrapperCallback： " + BleRequestImpl.this.notifyWrapperCallback);
                    if (BleRequestImpl.this.notifyWrapperCallback != null) {
                        BleRequestImpl.this.notifyWrapperCallback.onWriteSuccess(bluetoothGatt.getDevice(), bluetoothGattCharacteristic, i);
                    }
            ...
            synchronized (BleRequestImpl.this.locker) {
                if (bluetoothGatt != null) {
                    if (bluetoothGatt.getDevice() != null) {
                        BleLog.d(BleRequestImpl.TAG, bluetoothGatt.getDevice().getAddress() + " -- onCharacteristicChanged: " + (bluetoothGattCharacteristic.getValue() != null ? ByteUtils.toHexString(bluetoothGattCharacteristic.getValue()) : ""));
                        BleDevice bleDeviceInternal = BleRequestImpl.this.getBleDeviceInternal(bluetoothGatt.getDevice().getAddress());
                        if (BleRequestImpl.this.notifyWrapperCallback != null) {
        ...
        public void onDescriptorWrite(BluetoothGatt bluetoothGatt, BluetoothGattDescriptor bluetoothGattDescriptor, int i) {
            Log.d(BleRequestImpl.TAG, "onDescriptorWrite====================" + i);
            if (bluetoothGatt == null || bluetoothGatt.getDevice() == null) {
                return;
            }
            BleLog.d(BleRequestImpl.TAG, "write descriptor uuid:" + bluetoothGattDescriptor.getCharacteristic().getUuid() + "   ->status[" + i + "]");
            BleDevice bleDeviceInternal = BleRequestImpl.this.getBleDeviceInternal(bluetoothGatt.getDevice().getAddress());
            if (i == 0) {
                if (BleRequestImpl.this.notifyCharacteristics.size() <= 0 || BleRequestImpl.this.notifyIndex >= BleRequestImpl.this.notifyCharacteristics.size()) {
                    Log.d(BleRequestImpl.TAG, "onDescriptorWrite====setCharacteristicNotification is completed===");
                    if (BleRequestImpl.this.notifyWrapperCallback != null) {
                        if (Arrays.equals(bluetoothGattDescriptor.getValue(), BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE) || Arrays.equals(bluetoothGattDescriptor.getValue(), BluetoothGattDescriptor.ENABLE_INDICATION_VALUE)) {
                            Log.d(BleRequestImpl.TAG, "onDescriptorWrite onNotifySuccess===gatt.getDevice():" + bluetoothGatt.getDevice().getName());
                ...
                Log.d(BleRequestImpl.TAG, "onDescriptorWrite2:====================" + BleRequestImpl.this.notifyCharacteristics.size() + "> notifyIndex" + BleRequestImpl.this.notifyIndex);
        ...
        public void onDescriptorRead(BluetoothGatt bluetoothGatt, BluetoothGattDescriptor bluetoothGattDescriptor, int i) {
            ...
            BleLog.d(BleRequestImpl.TAG, "read descriptor uuid:" + bluetoothGattDescriptor.getCharacteristic().getUuid());
            BleDevice bleDeviceInternal = BleRequestImpl.this.getBleDeviceInternal(bluetoothGatt.getDevice().getAddress());
            if (i == 0) {
    ...
    public boolean connect(final T t) {
        ...
        HandlerCompat.postDelayed(this.handler, new Runnable() { // from class: com.wifiled.blelibrary.ble.BleRequestImpl.2
