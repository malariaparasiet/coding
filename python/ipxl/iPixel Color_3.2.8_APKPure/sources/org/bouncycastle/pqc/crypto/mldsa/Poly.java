...
class Poly {
    ...
    private void unpackZ(byte[] bArr) {
        ...
        if (dilithiumGamma1 != 131072) {
            ...
            while (i < 128) {
                ...
                int i3 = i * 5;
                int i4 = i3 + 2;
                setCoeffIndex(i2, ((bArr[i3] & UByte.MAX_VALUE) | ((bArr[i3 + 1] & UByte.MAX_VALUE) << 8) | ((bArr[i4] & UByte.MAX_VALUE) << 16)) & ErrorCode.ERR_UNKNOWN);
                int i5 = i2 + 1;
                setCoeffIndex(i5, (((bArr[i3 + 4] & UByte.MAX_VALUE) << 12) | ((bArr[i4] & UByte.MAX_VALUE) >> 4) | ((bArr[i3 + 3] & UByte.MAX_VALUE) << 4)) & ErrorCode.ERR_UNKNOWN);
                setCoeffIndex(i2, dilithiumGamma1 - getCoeffIndex(i2));
                setCoeffIndex(i5, dilithiumGamma1 - getCoeffIndex(i5));
                i++;
            }
            return;
        }
        while (i < 64) {
            ...
            setCoeffIndex(i6, dilithiumGamma1 - getCoeffIndex(i6));
            setCoeffIndex(i9, dilithiumGamma1 - getCoeffIndex(i9));
            setCoeffIndex(i11, dilithiumGamma1 - getCoeffIndex(i11));
            setCoeffIndex(i13, dilithiumGamma1 - getCoeffIndex(i13));
            i++;
        }
    }
    ...
    public void challenge(byte[] bArr, int i, int i2) {
        int i3;
        int i4;
        int i5;
        byte[] bArr2 = new byte[this.symmetric.stream256BlockBytes];
        SHAKEDigest sHAKEDigest = new SHAKEDigest(256);
        sHAKEDigest.update(bArr, i, i2);
        ...
        while (true) {
            ...
            j |= (bArr2[i6] & UByte.MAX_VALUE) << (i6 * 8);
            i6++;
        }
    ...
    public void decompose(Poly poly) {
        ...
        for (int i = 0; i < 256; i++) {
            int[] decompose = Rounding.decompose(getCoeffIndex(i), dilithiumGamma2);
    ...
    void packW1(byte[] bArr, int i) {
        int i2 = 0;
        if (this.engine.getDilithiumGamma2() != 95232) {
            if (this.engine.getDilithiumGamma2() == 261888) {
                while (i2 < 128) {
                    int i3 = i2 * 2;
                    bArr[i + i2] = (byte) ((getCoeffIndex(i3 + 1) << 4) | getCoeffIndex(i3));
                    i2++;
                }
                return;
            }
        ...
        while (i2 < 64) {
            ...
            int i5 = i2 * 4;
            int i6 = i5 + 1;
            bArr[i4] = (byte) (((byte) getCoeffIndex(i5)) | (getCoeffIndex(i6) << 6));
            int i7 = i5 + 2;
            bArr[i4 + 1] = (byte) (((byte) (getCoeffIndex(i6) >> 2)) | (getCoeffIndex(i7) << 4));
            bArr[i4 + 2] = (byte) ((getCoeffIndex(i5 + 3) << 2) | ((byte) (getCoeffIndex(i7) >> 4)));
            i2++;
        }
    }
    ...
    public void uniformBlocks(byte[] bArr, short s) {
        ...
        int rejectUniform = rejectUniform(this, 0, 256, bArr2, i);
        while (rejectUniform < 256) {
            ...
            rejectUniform += rejectUniform(this, rejectUniform, 256 - rejectUniform, bArr2, i);
        }
    }
    ...
    public void uniformEta(byte[] bArr, short s) {
        ...
        int dilithiumEta = this.engine.getDilithiumEta();
        if (this.engine.getDilithiumEta() == 2) {
            i = this.symmetric.stream256BlockBytes + Opcodes.I2D;
        } else {
            if (this.engine.getDilithiumEta() != 4) {
                throw new RuntimeException("Wrong Dilithium Eta!");
            }
            i = this.symmetric.stream256BlockBytes + Command.CMD_OTA_INQUIRE_DEVICE_IF_CAN_UPDATE;
        }
        ...
        Poly poly = this;
        int rejectEta = rejectEta(poly, 0, 256, bArr2, i2, dilithiumEta);
        while (rejectEta < 256) {
            Symmetric symmetric = poly.symmetric;
            symmetric.stream256squeezeBlocks(bArr2, 0, symmetric.stream256BlockBytes);
            rejectEta += rejectEta(poly, rejectEta, 256 - rejectEta, bArr2, poly.symmetric.stream256BlockBytes, dilithiumEta);
            poly = this;
        }
    }
    ...
    public void zPack(byte[] bArr, int i) {
        int dilithiumGamma1 = this.engine.getDilithiumGamma1();
        int i2 = 0;
        if (dilithiumGamma1 != 131072) {
            if (dilithiumGamma1 != 524288) {
            ...
            while (i2 < 128) {
                int i3 = i2 * 2;
                int coeffIndex = dilithiumGamma1 - getCoeffIndex(i3);
                int coeffIndex2 = dilithiumGamma1 - getCoeffIndex(i3 + 1);
                int i4 = (i2 * 5) + i;
                bArr[i4] = (byte) coeffIndex;
                bArr[i4 + 1] = (byte) (coeffIndex >> 8);
                bArr[i4 + 2] = (byte) (((byte) (coeffIndex >> 16)) | (coeffIndex2 << 4));
                bArr[i4 + 3] = (byte) (coeffIndex2 >> 4);
                bArr[i4 + 4] = (byte) (coeffIndex2 >> 12);
                i2++;
            }
            return;
        }
        while (i2 < 64) {
            int i5 = i2 * 4;
            int coeffIndex3 = dilithiumGamma1 - getCoeffIndex(i5);
            int coeffIndex4 = dilithiumGamma1 - getCoeffIndex(i5 + 1);
            int coeffIndex5 = dilithiumGamma1 - getCoeffIndex(i5 + 2);
            int coeffIndex6 = dilithiumGamma1 - getCoeffIndex(i5 + 3);
            int i6 = (i2 * 9) + i;
            bArr[i6] = (byte) coeffIndex3;
            bArr[i6 + 1] = (byte) (coeffIndex3 >> 8);
            bArr[i6 + 2] = (byte) (((byte) (coeffIndex3 >> 16)) | (coeffIndex4 << 2));
            bArr[i6 + 3] = (byte) (coeffIndex4 >> 6);
            bArr[i6 + 4] = (byte) (((byte) (coeffIndex4 >> 14)) | (coeffIndex5 << 4));
            bArr[i6 + 5] = (byte) (coeffIndex5 >> 4);
            bArr[i6 + 6] = (byte) (((byte) (coeffIndex5 >> 12)) | (coeffIndex6 << 6));
            bArr[i6 + 7] = (byte) (coeffIndex6 >> 2);
            bArr[i6 + 8] = (byte) (coeffIndex6 >> 10);
            i2++;
        }
    }
