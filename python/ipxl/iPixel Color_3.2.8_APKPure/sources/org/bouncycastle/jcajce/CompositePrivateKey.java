...
public class CompositePrivateKey implements PrivateKey {
    ...
    public byte[] getEncoded() {
        ...
        byte[] bArr = null;
        while (i < this.keys.size()) {
            bArr = Arrays.concatenate(bArr, PrivateKeyInfo.getInstance(this.keys.get(i).getEncoded()).getPrivateKey().getOctets());
            i++;
        }
        try {
            return new PrivateKeyInfo(new AlgorithmIdentifier(this.algorithmIdentifier), bArr).getEncoded(ASN1Encoding.DER);
        } catch (IOException e2) {
            throw new IllegalStateException("unable to encode composite private key: " + e2.getMessage());
        }
    }
