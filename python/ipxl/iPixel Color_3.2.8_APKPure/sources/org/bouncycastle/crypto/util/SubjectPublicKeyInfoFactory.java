    ...
    public static SubjectPublicKeyInfo createSubjectPublicKeyInfo(AsymmetricKeyParameter asymmetricKeyParameter) throws IOException {
        ...
        if (!(asymmetricKeyParameter instanceof ECPublicKeyParameters)) {
            ...
            if (asymmetricKeyParameter instanceof HSSPublicKeyParameters) {
                ...
                byte[] build = Composer.compose().u32str(hSSPublicKeyParameters.getL()).bytes(hSSPublicKeyParameters.getLMSPublicKey()).build();
                return new SubjectPublicKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.id_alg_hss_lms_hashsig), Arrays.concatenate(new byte[]{4, (byte) build.length}, build));
            }
            if (!(asymmetricKeyParameter instanceof LMSPublicKeyParameters)) {
                throw new IOException("key parameters not recognized");
            }
            byte[] build2 = Composer.compose().u32str(1).bytes((LMSPublicKeyParameters) asymmetricKeyParameter).build();
            return new SubjectPublicKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.id_alg_hss_lms_hashsig), Arrays.concatenate(new byte[]{4, (byte) build2.length}, build2));
        }
