...
public class EthereumIESEngine {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (this.cipher == null) {
            ...
            byte[] bArr10 = this.V;
            bArr3 = bArr;
            processBytes = bufferedBlockCipher.processBytes(bArr3, i + bArr10.length, (i2 - bArr10.length) - this.mac.getMacSize(), bArr9, 0);
            bArr4 = bArr9;
        }
        ...
        byte[] bArr12 = new byte[newInstance.getDigestSize()];
        newInstance.reset();
        newInstance.update(bArr2, 0, bArr2.length);
        newInstance.doFinal(bArr12, 0);
