    ...
    public void applyTo(EditingBuffer buffer) {
        ...
        if (i <= 0) {
            ...
            if (i3 > 0) {
                while (true) {
                    i2++;
                    findPrecedingBreak = JvmCharHelpers_androidKt.findPrecedingBreak(editingBuffer, selectionStart);
            ...
            while (true) {
                i2++;
                findPrecedingBreak = JvmCharHelpers_androidKt.findFollowingBreak(editingBuffer, selectionStart);
                if (findPrecedingBreak == -1) {
                    break;
                } else if (i2 >= i) {
                    break;
