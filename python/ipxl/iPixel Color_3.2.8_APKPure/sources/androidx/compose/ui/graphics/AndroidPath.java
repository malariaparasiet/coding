...
public final class AndroidPath implements Path {
    ...
    public int mo575getFillTypeRgk1Os() {
        if (this.internalPath.getFillType() == Path.FillType.EVEN_ODD) {
            return PathFillType.INSTANCE.m883getEvenOddRgk1Os();
        }
        return PathFillType.INSTANCE.m884getNonZeroRgk1Os();
    }

    @Override // androidx.compose.ui.graphics.Path
    /* renamed from: setFillType-oQ8Xj4U, reason: not valid java name */
    public void mo577setFillTypeoQ8Xj4U(int i) {
        Path.FillType fillType;
        android.graphics.Path path = this.internalPath;
        if (PathFillType.m879equalsimpl0(i, PathFillType.INSTANCE.m883getEvenOddRgk1Os())) {
    ...
    public void addRoundRect(RoundRect roundRect) {
        ...
        this.radii[0] = CornerRadius.m417getXimpl(roundRect.m492getTopLeftCornerRadiuskKHJgLs());
        this.radii[1] = CornerRadius.m418getYimpl(roundRect.m492getTopLeftCornerRadiuskKHJgLs());
        this.radii[2] = CornerRadius.m417getXimpl(roundRect.m493getTopRightCornerRadiuskKHJgLs());
        this.radii[3] = CornerRadius.m418getYimpl(roundRect.m493getTopRightCornerRadiuskKHJgLs());
        this.radii[4] = CornerRadius.m417getXimpl(roundRect.m491getBottomRightCornerRadiuskKHJgLs());
        this.radii[5] = CornerRadius.m418getYimpl(roundRect.m491getBottomRightCornerRadiuskKHJgLs());
        this.radii[6] = CornerRadius.m417getXimpl(roundRect.m490getBottomLeftCornerRadiuskKHJgLs());
        this.radii[7] = CornerRadius.m418getYimpl(roundRect.m490getBottomLeftCornerRadiuskKHJgLs());
        this.internalPath.addRoundRect(this.rectF, this.radii, Path.Direction.CCW);
    }

    @Override // androidx.compose.ui.graphics.Path
    /* renamed from: addPath-Uv8p0NA, reason: not valid java name */
    public void mo574addPathUv8p0NA(Path path, long offset) {
        Intrinsics.checkNotNullParameter(path, "path");
        android.graphics.Path path2 = this.internalPath;
        if (path instanceof AndroidPath) {
            path2.addPath(((AndroidPath) path).getInternalPath(), Offset.m442getXimpl(offset), Offset.m443getYimpl(offset));
            return;
        }
    ...
    public void mo578translatek4lQ0M(long offset) {
        this.mMatrix.reset();
        this.mMatrix.setTranslate(Offset.m442getXimpl(offset), Offset.m443getYimpl(offset));
    ...
    public boolean mo576opN5in7k0(Path path1, Path path2, int operation) {
        Path.Op op;
        Intrinsics.checkNotNullParameter(path1, "path1");
        Intrinsics.checkNotNullParameter(path2, "path2");
        if (PathOperation.m888equalsimpl0(operation, PathOperation.INSTANCE.m892getDifferenceb3I0S0c())) {
            op = Path.Op.DIFFERENCE;
        } else if (PathOperation.m888equalsimpl0(operation, PathOperation.INSTANCE.m893getIntersectb3I0S0c())) {
            op = Path.Op.INTERSECT;
        } else if (PathOperation.m888equalsimpl0(operation, PathOperation.INSTANCE.m894getReverseDifferenceb3I0S0c())) {
            op = Path.Op.REVERSE_DIFFERENCE;
        } else {
            op = PathOperation.m888equalsimpl0(operation, PathOperation.INSTANCE.m895getUnionb3I0S0c()) ? Path.Op.UNION : Path.Op.XOR;
        }
        android.graphics.Path path = this.internalPath;
